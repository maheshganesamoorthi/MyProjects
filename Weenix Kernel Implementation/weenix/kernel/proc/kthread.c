...
{
        KASSERT(NULL != p);
        dbg(DBG_PRINT, "(GRADING1A 3.a): thread can be created only if it has an associated process\n");
...
{
        KASSERT(NULL != kthr);
        dbg(DBG_PRINT, "(GRADING1A 3.b): there should be a thread to cancel\n");
...
kthread_exit(void *retval)
{
        ...
        dbg(DBG_PRINT, "(GRADING1A 3.c): curthr should not be in any queue\n");
        KASSERT(!curthr->kt_qlink.l_next && !curthr->kt_qlink.l_prev);
        dbg(DBG_PRINT, "(GRADING1A 3.c): queue should be empty\n");
        KASSERT(curthr->kt_proc == curproc);
        dbg(DBG_PRINT, "(GRADING1A 3.c): the thread should be in the current procedure\n");
