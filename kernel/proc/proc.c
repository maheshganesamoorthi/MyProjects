...
proc_create(char *name)
{
        ...
        if(p->p_pid!=0 && p->p_pid!=1 && p->p_pid!=2){
            dbg(DBG_PRINT, "(GRADING2B)\n");
            p->p_cwd = vget(curproc->p_cwd->vn_fs, curproc->p_cwd->vn_vno);
        }

        int file_desc_count;
        for(file_desc_count=0;file_desc_count<NFILES;file_desc_count++){
           p->p_files[file_desc_count] = NULL;
        }
        #endif
...
proc_cleanup(int status)
{
    ...
    proc_t *p;  
    int file_clean_count;
     for(file_clean_count=0;file_clean_count<NFILES;file_clean_count++){
        if(curproc->p_files[file_clean_count]!=NULL){
            dbg(DBG_PRINT, "(GRADING2B)\n");
            do_close(file_clean_count);
        }
    }   
