 ...
 *  $Id: vnode.c,v 1.11 2015/12/15 14:38:24 william Exp $
 */
...
special_file_read(vnode_t *file, off_t offset, void *buf, size_t count)
{
        ...
        if (S_ISCHR(file->vn_mode)){
            ...
            dbg(DBG_PRINT, "(GRADING2A 1.a)\n");
            int transfered = file->vn_cdev->cd_ops->read(file->vn_cdev,offset,buf,count);
            return transfered;
        }
        /*if(file->vn_bdev != NULL)
            dbg(DBG_PRINT, "(GRADING:480,special_file_read)\n");
            return -ENOTSUP;*/
        return 0;    
}
...
special_file_write(vnode_t *file, off_t offset, const void *buf, size_t count)
{
        ...
        if (S_ISCHR(file->vn_mode)){
            ...
            dbg(DBG_PRINT, "(GRADING2A 1.a)\n");
            int transfered = file->vn_cdev->cd_ops->write(file->vn_cdev,offset,buf,count);
            return transfered;
        }
        /*if(file->vn_bdev != NULL)
            dbg(DBG_PRINT, "(GRADING:506,special_file_write)\n");
            return -ENOTSUP;*/
        return 0;
}
